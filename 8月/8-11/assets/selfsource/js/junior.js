	...
	$(function(){
	$('.gongzuochengguo .content ul').width(532*$('.gongzuochengguo .content li').length+'px');
	$(".gongzuochengguo .tab a").mouseover(function(){
		$(this).addClass('active').siblings().removeClass('active');
		var index = $(this).index();
		number = index;
		var distance = -532*index;
	...
	if(auto ==1){
		...
		function autotab(){
			...
			var distance = -532*number;
			$('.gongzuochengguo .content ul').stop().animate({
				left:distance
			});
		}
		var tabChange = setInterval(autotab,2500);
		//鼠标悬停暂停切换
		$('.gongzuochengguo').mouseover(function(){
			clearInterval(tabChange);
		});
		$('.gongzuochengguo').mouseout(function(){
			tabChange = setInterval(autotab,2500);
		});
	};
